...
from picc_llm.app.tasks import run_training_task, generate_curriculum_task
    ...
    if request.method == "POST":
        ...
        if "register" in data:
            try:
                ...
                conditions, study_condition = generate_conditions(user.user_index)
                db.session.add_all(conditions)
                db.session.commit()

                user.study_condition = study_condition
...
def environment():
    ...
    if request.method == "GET":
        ...
        # 3. CONFIG INITIALISATION (Pre-flight)
        # If no active config exists, either trigger AI generation or set a blank default.
        if learning_handler.learner.active_environment_config is None:
            if user.study_condition == "ai":
                app.logger.info(
                ...
                app.logger.info(
                    f"User {user_index}: Baseline condition. Setting blank default config."
                )
                learning_handler.learner.active_environment_config = {
                    "width": 10, "height": 10, "objects": {}, "rewards": {}
                }
                db.session.commit()
