...
window.onload = function() {
    ...
    if (isValidConfig) {
        ...
        if (initialConfig.rewards) {
            for (const [name, tier] of Object.entries(initialConfig.rewards)) {
                let select = document.querySelector(`.obj-reward-select[data-name="${name}"]`);
                if (!select) select = document.querySelector(`.obj-reward-select[data-name="${name.toUpperCase()}"]`);
...
/**
 * Resets all form inputs back to the AI-suggested initialConfig.
 */
function resetToDefault() {
    ...
    document.querySelectorAll('.obj-counter').forEach(el => el.value = 0);
    document.querySelectorAll('.obj-reward-select').forEach(el => el.value = 'none');
...
function submitCurriculum() {
    const allNone = [...document.querySelectorAll('.obj-reward-select')].every(s => s.value === 'none');
    if (allNone) {
        const modal = new bootstrap.Modal(document.getElementById('rewardWarningModal'));
        document.getElementById('confirmSubmitBtn').onclick = () => { modal.hide(); doSubmit(); };
        modal.show();
        return;
    }
    doSubmit();
...
function doSubmit() {
    ...
    const rewards = {};
    document.querySelectorAll('.obj-reward-select').forEach(select => {
        rewards[select.getAttribute('data-name')] = select.value;
    });
